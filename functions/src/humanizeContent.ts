...
function calculateDynamicTokens(text: string): number {
  if (!text) return 1000;
...
function cleanPunctuationAndWhitespace(str: string): string {
  if (!str) return '';
  return str
...
function canonicalizeModelName(modelName: string): string {
...
function matchesProvider(model: string, keys: { gemini?: string | null; openai?: string | null; openrouter?: string | null }): boolean {
  if (keys.openrouter) return true; // OpenRouter handles all models
...
// --- Main Handler ---
export async function humanizeContentHandler(req: AuthenticatedRequest, res: Response) {
  ...
  if (req.apiKeyId) {
    ...
    if (keyDoc.exists) {
      ...
      if (!mode && keyData?.defaultMode) mode = keyData.defaultMode;
      if (!voice_profile_id && keyData?.defaultVoiceProfileId) voice_profile_id = keyData.defaultVoiceProfileId;
    }
  }
  ...
  if (reflection_level === 'advanced') reflectionMultiplier = 1.5;
  if (reflection_level === 'maximum') reflectionMultiplier = 2.0;

  let voiceMultiplier = 1.0;
  let voiceProfileFingerprint: any = null;
  ...
  try {
    await db.runTransaction(async (transaction) => {
      ...
      if (userData && userData.email && !userEmail) userEmail = userData.email;
      ...
      if (currentCredits < requiredCredits) throw new Error('Insufficient credits');
  ...
  const writeStreamEvent = (data: any) => {
    if (stream) res.write(JSON.stringify(data) + '\n');
  };
  ...
  try {
    ...
    }[mode as string] || { temperature: 0.85, frequencyPenalty: 0.4, presencePenalty: 0.3 };

    const agentLogs: any[] = [];
    const runAgent = async (agentNum: number, agentName: string, sysPrompt: string, usrPrompt: string, modelToUse: string, isJson = false) => {
      // Check cancel status?
      const jDoc = await jobRef.get();
      if (jDoc.data()?.status === 'cancelled') throw new Error('Job cancelled');

      writeStreamEvent({ event: 'agent_start', agentNumber: agentNum, agentName, model: modelToUse, createdAt: Date.now() });

      const fullSysPrompt = sysPrompt + `\n\n--- MODE INSTRUCTIONS ---\n${MODE_INSTRUCTIONS[mode as string] || MODE_INSTRUCTIONS.standard}`;
    ...
    if (voiceProfileFingerprint) personaInfo += ` Vault Persona: ${JSON.stringify(voiceProfileFingerprint)}.`;
    ...
    let maxLoops = 1;
    if (reflection_level === 'advanced') maxLoops = 2;
    if (reflection_level === 'maximum') maxLoops = 3;
    ...
    if (creditsLocked) {
      ...
      await db.collection('creditTransactions').add({ userId, type: 'refund', amount: requiredCredits, jobId, taskType: 'humanization', mode, wordsIn: wordCount, createdAt: FieldValue.serverTimestamp() });
    }
