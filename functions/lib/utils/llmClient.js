...
const tokenLogger_1 = require("./tokenLogger");
...
async function callLlm(options) {
    ...
    const { model, systemPrompt, userPrompt, jsonMode = false, temperature, agentName = 'Unknown Agent', userId, jobId, userLlmKeys } = options;
    ...
    try {
        ...
        if (selectedProvider === 'gemini') {
            ...
            const config = { maxOutputTokens: isPro ? 16384 : 8192 };
            if (jsonMode)
        ...
        const metrics = { tokensIn, tokensOut, thinkingTokens, latencyMs, costUsd, status: 'completed' };
        await (0, tokenLogger_1.logTokenUsage)({ userId, jobId, agentName, model, tokensIn, tokensOut, thinkingTokens, costUsd, latencyMs }).catch(console.error);
